# sfc/pipelines/rbcp_pipeline.py

"""
RbCP pipeline: sweeps p2p and bw_signal and computes MSE bounds and metrics.
"""
...
def run(config_path: str, *, save: bool = True):
    """
    Run the RbCP experiment.

    Notes
    -----
    This pipeline is "hybrid": the config controls the experiment settings,
    while model-derived variables are computed inside the code.
    """
    ...
    if save:
        save_dat_with_metadata(
            ...
            experiment="rbcp",
            config=cfg,
            header="upBound_MSE\tlowBound_MSE\tMSE_rbcp\tMSE\tMSExy\tMc\tBins\tM\tbw\tsensors\tsnr_dB\tW\tsampling_rate\tN\ttau\tp2p\tmean_p2p",
            fmt="%.8f"
        )

    return {"results": all_results, "config": cfg}
