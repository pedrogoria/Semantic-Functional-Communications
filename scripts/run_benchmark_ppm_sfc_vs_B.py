...
Runner for the fair comparison experiment:
...
- runs the fair-comparison pipeline
- saves the .dat output
- saves metadata and a copy of the config
- generates the main MSE-vs-B plot
...
def save_data(df, cfg, output_dir, timestamp):
    ...
    base_name = f"benchmark_ppm_sfc_vs_B_{timestamp}"
...
def generate_plot(df, cfg, output_dir, timestamp):
    """
    Generate the fair-comparison MSE-vs-B plot.
    """
    ...
    if "mse_benchmark_fdma" in df.columns and df["mse_benchmark_fdma"].notna().any():
        plt.plot(
            df["B"],
            df["mse_benchmark_fdma"],
            linestyle="-",
            linewidth=2,
            label=labels.get("benchmark_fdma", "Benchmark + FDMA")
        )

    if "mse_ppm_fdma" in df.columns and df["mse_ppm_fdma"].notna().any():
        plt.plot(
            df["B"],
            df["mse_ppm_fdma"],
            linestyle="--",
            linewidth=2,
            label=labels.get("ppm_fdma", "PPM + FDMA")
        )
    ...
    if "mse_sfc_sed" in df.columns and df["mse_sfc_sed"].notna().any():
        plt.plot(
            df["B"],
            df["mse_sfc_sed"],
            linestyle=(0, (3, 1, 1, 1)),
            linewidth=2,
            label=labels.get("sfc_sed", "SFC + SED")
        )
    ...
    plt.title(cfg.get("figure", {}).get(
        "title",
        "Fair comparison versus B: Benchmark+FDMA, PPM+FDMA, RbCP_time, SFC, and SFC+SED"
    ))

    for fmt in cfg["output"]["formats"]["plot"]:
        plt.savefig(
            os.path.join(output_dir, f"benchmark_ppm_sfc_vs_B_{timestamp}.{fmt}"),
            bbox_inches="tight"
        )
...
def save_metadata(cfg, df, output_dir, timestamp, config_path):
    ...
    with open(meta_path, "w") as f:
        f.write("Experiment: benchmark_ppm_sfc_vs_B\n")
        ...
        if "N0_derived" in df.columns:
            f.write(f"N0_derived min: {df['N0_derived'].min()}\n")
            f.write(f"N0_derived max: {df['N0_derived'].max()}\n\n")

        if "mse_benchmark_fdma" in df.columns:
            f.write(f"mse_benchmark_fdma min: {df['mse_benchmark_fdma'].min()}\n")
            f.write(f"mse_benchmark_fdma max: {df['mse_benchmark_fdma'].max()}\n\n")

        if "mse_ppm_fdma" in df.columns:
            f.write(f"mse_ppm_fdma min: {df['mse_ppm_fdma'].min()}\n")
            f.write(f"mse_ppm_fdma max: {df['mse_ppm_fdma'].max()}\n\n")
        ...
        if "mse_sfc_sed" in df.columns:
            f.write(f"mse_sfc_sed min: {df['mse_sfc_sed'].min()}\n")
            f.write(f"mse_sfc_sed max: {df['mse_sfc_sed'].max()}\n\n")

        if "valid_period_fraction_sfc_sed" in df.columns:
            f.write(f"valid_period_fraction_sfc_sed min: {df['valid_period_fraction_sfc_sed'].min()}\n")
            f.write(f"valid_period_fraction_sfc_sed max: {df['valid_period_fraction_sfc_sed'].max()}\n\n")

        if "num_valid_trials_sfc_sed" in df.columns:
            f.write(f"num_valid_trials_sfc_sed min: {df['num_valid_trials_sfc_sed'].min()}\n")
            f.write(f"num_valid_trials_sfc_sed max: {df['num_valid_trials_sfc_sed'].max()}\n\n")
...
def run_benchmark_ppm_sfc_vs_B(config_path):
    """
    ...
    print("[INFO] Running benchmark_ppm_sfc_vs_B...\n")
    print(f"[INFO] Figure title: {cfg.get('figure', {}).get('title', 'benchmark_ppm_sfc_vs_B')}")
...
def main():
    ...
    run_benchmark_ppm_sfc_vs_B(args.config)
